...
for pos in full_path:
  ...
  for key in real_node_positions:
    ...
    p1 = (max(p1[0], real_node_positions[key][0] - distances[counter][key]), max(p1[1], real_node_positions[key][1] - distances[counter][key]))
    p2 = (min(p2[0], real_node_positions[key][0] + distances[counter][key]), min(p2[1], real_node_positions[key][1] + distances[counter][key]))
    print (real_node_positions[key], distances[counter][key], p1, p2)
  ...
  #fig1.savefig('img/localization_mm_'+str(counter).zfill(3)+'.png', dpi=180, bbox_inches='tight')
