...
uint8_t WM8978_Init(void)
{
	...
	wm8979_interface();
	uint8_t ret = WM8978_Write_Reg(1,0X3B);	//R1,MICEN??Ϊ1(MICʹ?),BIASEN??Ϊ1(ģ?????),VMIDSEL[1:0]??Ϊ:11(5K)
	ret |= WM8978_Write_Reg(2,0X1B0);	//R2,ROUT1,LOUT1???ʹ?(??????Թ??),BOOSTENR,BOOSTENLʹ?
	ret |= WM8978_Write_Reg(3,0X6C);	//R3,LOUT2,ROUT2???ʹ?(???ȹ??),RMIX,LMIXʹ?	//0x6c
	//WM8978_Write_Reg(6,0);		//R6,MCLK???????
	ret |= WM8978_Write_Reg(43,1<<4);	//R43,INVROUT2???,???????
	ret |= WM8978_Write_Reg(47,1<<8);	//R47??,PGABOOSTL,?ͨ??MIC??20????
	ret |= WM8978_Write_Reg(48,1<<8);	//R48??,PGABOOSTR,?ͨ??MIC??20????
	ret |= WM8978_Write_Reg(49,1<<1);	//R49,TSDEN,?????????? 
	ret |= WM8978_Write_Reg(10,1<<3);	//R10,SOFTMUTE?ر?128x???,???NR 
	ret |= WM8978_Write_Reg(14,1<<3);	//R14,ADC 128x????
	//wm8979_loopback();
	return ret;
} 
//WM8978写寄存器
...
{ 
	...
	esp_err_t ret = hal_i2c_master_mem_write(0,WM8978_ADDR,buf[0],buf+1,1);
